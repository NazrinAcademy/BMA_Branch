...
def check_expiring_subscriptions():
    today = now().date()
    warning_date = today + timedelta(days=7)  # 7 days before expiry

    expiring_subscriptions = SubscriptionPlan.objects.filter(
        is_active=True,
        start_date__lte=warning_date,
        start_date__gt=today
    )

    for subscription in expiring_subscriptions:
        admin = subscription.admin
        message = f"Your subscription '{subscription.plan_name}' will expire on {subscription.expiry_date()}."
