...
#     return async_to_sync(async_get_category_and_subcategory)(request)

# async def async_get_category_and_subcategory(request):
#     if request.method == 'GET':
#         try:
#             data = json.loads(request.body.decode('utf-8'))
...
#             subcategories = await sync_to_async(SubCategory.objects.filter(category_name=category))()
#             subcategory_details = [
...
#     return async_to_sync(async_delete_category_and_subcategory)(request)

# async def async_delete_category_and_subcategory(request):
#     if request.method == 'DELETE':
#         try:
#             data = json.loads(request.body.decode('utf-8'))
#             if 'category_id' not in data:
...
#             subcategories = await sync_to_async(SubCategory.objects.filter(category_name=category))()
#             await sync_to_async(subcategories.delete)()
...
async def async_create_product(data):
    try:
        ...
        if base64_img:
            base64_img = re.sub(r'\s+', '', base64_img)  
            base64_img = re.sub(r'[^A-Za-z0-9+/=]', '', base64_img) 
...
def update_product(request):
    if request.method == "PUT":
        try:
            ...
            return async_to_sync(async_update_product)(
                product_id, customer_id, product_name, category_name,
                sub_category_name, brand_name, unit, product_code,
                bar_qr_code, description, img
            )
        except json.JSONDecodeError:
            return JsonResponse({"error": "Invalid JSON format"}, status=400)
...
async def async_update_product(
    ...
    try:
        ...
        # Check for existing values individually
        existing_product = await sync_to_async(
    lambda: ProductDetails.objects.filter(sub_category_name=sub_category_name,brand_name=brand_name,product_code=product_code).exclude(id=product_id).exists())()
        ...
        product.bar_qr_code = bar_qr_code
        ...
        response_data = {
            ...
            "product_id": product.id,
            "customer_id": product.customer_id.id,
            "product_name": product.product_name,
            "category_name": product.category_name,
            "sub_category_name": product.sub_category_name,
            "brand_name": product.brand_name,
...
async def async_create_subscription_plan(request):
    if request.method == 'POST':
        try:
            ...
            # Fetch admin asynchronously
            admin = await sync_to_async(Admin.objects.filter(id=admin_id).first)()

            if not admin:
                return JsonResponse({"message": "Admin not found"}, status=404)

            # Calculate created_at date from the data (ensure proper format)
            ...
            # Create and save the subscription plan
            ...
            # Calculate the expiry date by adding duration_days to created_at
...
#         return JsonResponse({"message": "Notifications sent successfully", "data": notifications_sent})
...
# Run this function via cron or Celery
...
def razorpay_payment_view(request):
    if request.method == "POST":
        try:
            ...
            amount_in_paise = int(float(amount) * 100)  # Convert rupees to paise
