...
# --------------GET ALL E-WAY BILL DETAILS------------------


@csrf_exempt
def get_all_ewaybill_details(request):
    return async_to_sync(async_get_all_ewaybill_details)(request)

async def async_get_all_ewaybill_details(request):
    if request.method == 'GET':
        try:
            ewaybill_details_list = await sync_to_async(lambda: list(EWayBill.objects.all().values()))()

            if not ewaybill_details_list:
                return JsonResponse({'error': 'No ewaybill details found.'}, status=404)

            return JsonResponse({'supplier_details_list': ewaybill_details_list}, status=200)

        except Exception as e:
            return JsonResponse({'error': 'Something went wrong.', 'details': str(e)}, status=500)

    return JsonResponse({'error': 'Invalid method. Please use GET.'}, status=405)
...
# ----------------GET ALL E-INVOICE DETAILS -----------------------------

@csrf_exempt
def get_all_einvoice_details(request):
    return async_to_sync(async_get_all_einvoice_details)(request)

async def async_get_all_einvoice_details(request):
    if request.method == 'GET':
        try:
            einvoice_details_list = await sync_to_async(lambda: list(EInvoice.objects.all().values()))()

            if not einvoice_details_list:
                return JsonResponse({'error': 'No einvoice details found.'}, status=404)

            return JsonResponse({'supplier_details_list': einvoice_details_list}, status=200)
...
def get_all_category(request):
    return async_to_sync(async_get_all_category)(request)

async def async_get_all_category(request):
    if request.method == 'GET':
        try:
            categories = await sync_to_async(lambda: list(Category.objects.all().values()))()
...
def get_all_subcategories(request):
    return async_to_sync(async_get_all_subcategories)(request)

async def async_get_all_subcategories(request):
    if request.method == 'GET':
        try:
            subcategories = await sync_to_async(lambda: list(SubCategory.objects.all().values()))()

            if not subcategories:
                return JsonResponse({"message": "No subcategories found."}, status=404)

            return JsonResponse(subcategories, safe=False, status=200)
...
def get_all_brands(request):
    if request.method == 'GET':
        return async_to_sync(async_get_all_brands)(request)

async def async_get_all_brands(request):
    try:
        brands = await sync_to_async(list)(Brand.objects.all())
        
        if not brands:
            return JsonResponse({"message": "No brands found."}, status=404)

        brand_list = [{"id": brand.id, "brand_name": brand.brand_name} for brand in brands]

        return JsonResponse(brand_list, safe=False, status=200)
...
def get_all_units(request):
    if request.method == 'GET':
        return async_to_sync(async_get_all_units)(request)

async def async_get_all_units(request):
    try:
        ...
        return JsonResponse(unit_list, safe=False, status=200)
    except Exception as e:
        return JsonResponse({"error": str(e)}, status=500)
...
async def async_create_product(data):
    try:
        required_fields = ['customer_id', 'product_name', 'category_name', 'sub_category_name', 'brand_name', 'unit', 'product_model']
        missing_fields = [field for field in required_fields if field not in data]

        if missing_fields:
            return JsonResponse({"error": f"Missing fields: {', '.join(missing_fields)}"}, status=400)

        existing_product = await sync_to_async(lambda: ProductDetails.objects.filter(sub_category_name=data['sub_category_name'], brand_name=data['brand_name'], product_model=data['product_model']).exists())()
...
async def async_get_all_products(request):
    ...
    if request.method == "GET":
        try:
            products = await sync_to_async(lambda: list(ProductDetails.objects.all().values()))()
            if not products:
...
@csrf_exempt
def get_all_price_details(request):
    return async_to_sync(async_get_all_price_details)(request)

async def async_get_all_price_details(request):
    if request.method == 'GET':
        try:
            price_details_list = await sync_to_async(lambda: list(PriceDetails.objects.all().values()))()

            if not price_details_list:
                return JsonResponse({"error": "No price details found."}, status=404)

            return JsonResponse({"price_details": price_details_list}, status=200)
...
#---------------GET ALL GST DETAILS----------------------------
@csrf_exempt
def get_all_gst_details(request):
    return async_to_sync(async_get_all_gst_details)(request)

async def async_get_all_gst_details(request):
    if request.method == 'GET':
        try:
            gst_details_list = await sync_to_async(lambda: list(GSTDetails.objects.all().values()))()

            if not gst_details_list:
                return JsonResponse({'error': 'No GST details found.'}, status=404)

            return JsonResponse({'gst_details': gst_details_list}, status=200)
...
@csrf_exempt
def get_all_stock_details(request):
    return async_to_sync(async_get_all_stock_details)(request)

async def async_get_all_stock_details(request):
    if request.method == 'GET':
        try:
            stock_details_list = await sync_to_async(lambda: list(StockDetails.objects.all().values()))()

            if not stock_details_list:
                return JsonResponse({'error': 'No Stock details found.'}, status=404)

            return JsonResponse({'stock_details_list': stock_details_list}, status=200)
...
def get_all_employee_details(request):
    return async_to_sync(async_get_all_employee_details)(request)

async def async_get_all_employee_details(request):
    if request.method == 'GET':
        try:
            employee_details_list = await sync_to_async(lambda: list(Customer.objects.all().values()))()

            if not employee_details_list:
                return JsonResponse({'error': 'No employee details found.'}, status=404)

            return JsonResponse({'employee_details_list': employee_details_list}, status=200)
...
@csrf_exempt
def get_all_admin_details(request):
    return async_to_sync(async_get_all_admin_details)(request)

async def async_get_all_admin_details(request):
    if request.method == 'GET':
        try:
            admin_details_list = await sync_to_async(lambda: list(Admin.objects.all().values()))()

            if not admin_details_list:
                return JsonResponse({'error': 'No admin details found.'}, status=404)

            return JsonResponse({'admin_details_list': admin_details_list}, status=200)
...
def get_all_subscription_details(request):
    return async_to_sync(async_get_all_subscription_details)(request)

async def async_get_all_subscription_details(request):
    if request.method == 'GET':
        try:
            subscription_details_list = await sync_to_async(lambda: list(SubscriptionPlan.objects.all().values()))()

            if not subscription_details_list:
                return JsonResponse({'error': 'No Subcription plan details found.'}, status=404)

            return JsonResponse({'subscription_details_list': subscription_details_list}, status=200)
...
@csrf_exempt
def get_all_license_details(request):
    return async_to_sync(async_get_all_license_details)(request)

async def async_get_all_license_details(request):
    if request.method == 'GET':
        try:
            license_details_list = await sync_to_async(lambda: list(License.objects.all().values()))()

            if not license_details_list:
                return JsonResponse({'error': 'No license details found.'}, status=404)

            return JsonResponse({'license_details_list': license_details_list}, status=200)
...
@csrf_exempt
def get_all_supplier_details(request):
    return async_to_sync(async_get_all_supplier_details)(request)

async def async_get_all_supplier_details(request):
    if request.method == 'GET':
        try:
            supplier_details_list = await sync_to_async(lambda: list(SupplierList.objects.all().values()))()

            if not supplier_details_list:
                return JsonResponse({'error': 'No supplier details found.'}, status=404)

            return JsonResponse({'supplier_details_list': supplier_details_list}, status=200)
...
async def async_create_customer(request):
    if request.method == "POST":
        try : 
            ...
            required_fields = ["customer_name","mobile_no","email","address","area","pin_code","state","opening_balance","gst_no"]
...
def get_all_customer_details(request):
    return async_to_sync(async_get_all_customer_details)(request)

async def async_get_all_customer_details(request):
    if request.method == 'GET':
        try:
            customer_details_list = await sync_to_async(lambda: list(CustomerList.objects.all().values()))()

            if not customer_details_list:
                return JsonResponse({'error': 'No customer details found.'}, status=404)

            return JsonResponse({'supplier_details_list': customer_details_list}, status=200)
...
async def async_update_customer(request):
    if  request.method == "PUT":
        ...
        try:
            ...
            required_fields = ['customer_name','mobile_no','email','address','area','pin_code', 'state', 'opening_balance','gst_number']
...
def get_all_users(request):
    return async_to_sync(async_get_all_users)(request)

async def async_get_all_users(request):
    if request.method == "GET":
        try:
...
def delete_all_users(request):
    return async_to_sync(async_delete_all_users)(request)

async def async_delete_all_users(request):
    if request.method == "DELETE":
        try:
...
def search_customer(request):
    return async_to_sync(async_search_customer)(request)


async def async_search_customer(request):
    customer_name = request.GET.get('name')
    customers = await sync_to_async(list)(CustomerList.objects.filter(customer_name__icontains=customer_name))
...
def get_customer_details(request):
    return async_to_sync(async_get_customer_details)(request)


async def async_get_customer_details(request):
    customer_id = request.GET.get('customer_id')
    customer = await sync_to_async(CustomerList.objects.filter(id=customer_id).first)()
    if customer:
        data = {
...
def search_product(request):
    return async_to_sync(async_search_product)(request)


async def async_search_product(request):
    product_name = request.GET.get('name')
    products = await sync_to_async(list)(ProductDetails.objects.filter(product_name__icontains=product_name))
...
def get_product_details(request):
    return async_to_sync(async_get_product_details)(request)


async def async_get_product_details(request):
    product_id = request.GET.get('product_id')
    product = await sync_to_async(ProductDetails.objects.filter(id=product_id).first)()
    if product:
        sub_category = await sync_to_async(SubCategory.objects.filter(subcategory_name=product.sub_category_name).first)()
        hsn_sac_code = sub_category.hsn_sac_code if sub_category else ''

        price_details = await sync_to_async(PriceDetails.objects.filter(product=product).first)()
        discount_percentage = price_details.discount if price_details else 0

        gst_details = await sync_to_async(GSTDetails.objects.filter(product=product).first)()
...
#     return async_to_sync(async_add_sale_with_calculation)(request)

# async def async_add_sale_with_calculation(request):
#     if request.method == 'POST':
#         try:
#             data = json.loads(request.body)
#             customer_id = data.get('customer_id')
...
#             received = Decimal(data.get('received', 0))
...
#             total_amount_before_tax = Decimal(0)
#             total_discount = Decimal(0)
#             total_cgst = Decimal(0)
#             total_sgst = Decimal(0)
#             grant_total = Decimal(0)

#             last_sale = await sync_to_async(Sale.objects.last)()
#             invoice_no = last_sale.id + 1 if last_sale else 1

#             due_date = parse_date(due_date) if due_date else date.today()
#             invoice_date = date.today()

#             for product_id, quantity in zip(product_ids, quantities):
#                 product = await sync_to_async(ProductDetails.objects.filter(id=product_id).first)()
#                 if not product:
#                     return JsonResponse({'error': f'Product with ID {product_id} not found'})
...
#                 price_details = await sync_to_async(PriceDetails.objects.filter(product=product).first)()
#                 sale_price = Decimal(price_details.sale_price if price_details else 0)
#                 discount_percentage = Decimal(price_details.discount if price_details else 0)

#                 gst_details = await sync_to_async(GSTDetails.objects.filter(product=product).first)()
#                 cgst_percentage = Decimal(gst_details.cgst_percentage if gst_details else 0)
#                 sgst_percentage = Decimal(gst_details.sgst_percentage if gst_details else 0)

#                 quantity = int(quantity)
#                 rate = sale_price * quantity
#                 discount_value = (rate * discount_percentage) / 100
...
#                     total_amount_2=grant_total,
#                     received=received,
#                     balance=grant_total - received,
...
def store_sale(request):
    if request.method == 'POST':
        try:
            ...
            product_names = ', '.join([product['product_name'] for product in product_list])
            hsn_codes = ', '.join([product['hsn_sac_code'] for product in product_list])
            quantities = ', '.join([str(product['quantity']) for product in product_list if isinstance(product['quantity'], (int, Decimal))])
            rates = ', '.join([str(product['rate']) for product in product_list if isinstance(product['rate'], (int, Decimal))])
            discount_percentages = ', '.join([str(product['discount_percentage']) for product in product_list if isinstance(product['discount_percentage'], (int, Decimal))])
            discount_values = ', '.join([str(product['discount_value']) for product in product_list if isinstance(product['discount_value'], (int, Decimal))])
            cgst_percentages = ', '.join([str(product['cgst_percentage']) for product in product_list if isinstance(product['cgst_percentage'], (int, Decimal))])
            cgst_values = ', '.join([str(product['cgst_value']) for product in product_list if isinstance(product['cgst_value'], (int, Decimal))])
            sgst_percentages = ', '.join([str(product['sgst_percentage']) for product in product_list if isinstance(product['sgst_percentage'], (int, Decimal))])
            sgst_values = ', '.join([str(product['sgst_value']) for product in product_list if isinstance(product['sgst_value'], (int, Decimal))])
            total_amounts = ', '.join([str(product['total_amount']) for product in product_list if isinstance(product['total_amount'], (int, Decimal))])


            grant_total = sum(Decimal(product['total_amount']) for product in product_list)
            balance = grant_total - received
            ...
            sale = Sale(
                ...
                hsn_sac_code=hsn_codes,
                quantity=quantities,
                rate=rates,
                discount_percentage=discount_percentages,
                discount_value=discount_values,
                cgst_percentage=cgst_percentages,
                cgst_value=cgst_values,
                sgst_percentage=sgst_percentages,
                sgst_value=sgst_values,
            ...
            return JsonResponse({"message": "Sale Added Successfully"}, status= 200)
...
def get_sales(request):
    return async_to_sync(async_get_sales)(request)


async def async_get_sales(request):
    if request.method == 'GET':
        sales = await sync_to_async(list)(Sale.objects.all())
        sales_data = [
            {
                "invoice_no": sale.invoice_no,
                "date": sale.invoice_date,
                "total_amount": float(sale.total_amount_2),
                "payment_due": float(sale.balance),
...
def edit_sale(request):
    return async_to_sync(async_edit_sale)(request)


async def async_edit_sale(request):
    if request.method == 'PUT':
        try:
            data = json.loads(request.body)
            ...
            payment_due = Decimal(data.get('payment_due', 0))
            payment_method = data.get('payment_method')
            status = data.get('status')

            sale = await sync_to_async(Sale.objects.filter(invoice_no=invoice_no).first)()

            if not sale:
                return JsonResponse({"error": "Sale not found"}, status=404)

            if date:
                sale.invoice_date = datetime.strptime(date, '%Y-%m-%d')


            sale.total_amount_2 = total_amount
            sale.balance = payment_due
            sale.payment_type = payment_method
    ...
    return JsonResponse({"error": "Invalid request method"}, status=400)
# ----------------------DELETE ADD_SALE DATA----------------------------

@csrf_exempt
def delete_sale(request):
    return async_to_sync(async_delete_sale)(request)


async def async_delete_sale(request):
    if request.method == 'DELETE':
        try:
            data = json.loads(request.body)
            invoice_no = data.get('invoice_no')
