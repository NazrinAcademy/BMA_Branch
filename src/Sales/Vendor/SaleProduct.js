...
import { ArrowLeft, Search, CircleX, Pencil, ChevronDown, Plus } from 'lucide-react-native'; // Import Plus icon
...
const SaleProduct = ({ navigation, customizedProducts, setCustomizedProducts }) => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (fetchedData && Array.isArray(fetchedData.data)) {
          console.log("colorshadeDetails:", fetchedData.data[0].colorshadeDetails);
  
          // Transform API data to match expected format
          const formattedProducts = fetchedData.data.map((item) => ({
            id: item._id,
            productType: item.productname || "Unknown Product",
            modelNo: item.modelno || "No Model",
            size: `${item.height}*${item.width}`,
            skinColorShade: Array.isArray(item.colorshadeDetails) 
              ? item.colorshadeDetails[0]?.colorshadename || "N/A"  // If it's an array, get the first element's colorshadename
              : item.colorshadeDetails?.colorshadename || "N/A",    // If it's an object, access directly
                  ...
                  <View style={styles.fieldset}>
                    ...
                    <View style={styles.inputWithIcon}>
                      <TextInput
                        ...
                        onChangeText={(text) => {
                          // Remove "mm" before updating the state
