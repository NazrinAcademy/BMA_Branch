...
import { addProduct, getColorshadeByname, updateProduct } from '../../Utils/apiService';
...
const [selectedColorShade, setSelectedColorShade] = useState("");
  ...
  useEffect(() => {
    if (route.params?.product) {
      ...
      setMembraneShade(product.membraneShade || '');
      setSelectedColorShade(product.colorshadename || '');
      setColorshadesid(product.colorshadesid || '');
      setColorshadeid(product.colorshadeid || '');
      setPrl(product.pr1 || '');
      setPr2(product.pr2 || '');
      setPr3(product.pr3 || '');
      setPr4(product.pr4 || '');
      setQantityprocessingtime(product.qantityprocessingtime || '');
    }
  ...
  const newProductSubmitHandler = async () => {
    try {
      const response = await updateProduct(
        ...
        modelno,
        thickness,
        colorshadesid,  // Corrected from colorshadeid
        pr1,
        pr2,
        pr3,
        pr4,
  ...
  const handleProductTypeChange = async (type) => {
    try {
      ...
      if (type !== '') {
        const products = await getColorshadeByname(type);
        console.log('Products:', products);
        
        if (products?.data?.length > 0) {
          setColorshadeid(products.data[0]._id);
      ...
      console.error('Error fetching color shade:', error);
    }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.headerContainer}>
        ...
        <Text style={styles.headerText}> Updateproduct</Text>
   +   </View>
