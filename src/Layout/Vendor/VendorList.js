...
const VendorList = () => {
  ...
  const getFilteredData = () => {
    if (!searchText.trim()) return vendorData;

    return vendorData.filter(
      (vendor) =>
        vendor.shopname.toLowerCase().includes(searchText.toLowerCase()) ||
        vendor.mobileno.includes(searchText)
    );
  };
  ...
  const renderVendor = ({ item }) => (
    <TouchableOpacity
      style={styles.memberCard}
      onPress={() =>
        navigation.navigate('updatevendor', { member: item, role: 'Vendor' })
      }
    >
      <View style={styles.memberInfo}>
        <Text style={styles.memberName}>{item.shopname}</Text>
  ...
  return (
    <View style={styles.container} key="vendor-list">
      <View style={styles.header}>
        <TouchableOpacity onPress={handleArrowLeftClick}>
          <ArrowLeft size={24} color="#202020" />
        </TouchableOpacity>
