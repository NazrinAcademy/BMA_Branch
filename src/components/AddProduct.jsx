...
const AddProduct = () => {
  ...
  useEffect(() => {
    if (brand && productModel && subCategory) {
      setProductName(`${subCategory} ${brand} ${productModel}`);
    }
...
const handleSearchLocation = (searchText) => {
  ...
  if (searchText.trim() === "") {
    ...
    const filtered = personalDetails
      .map((person) => person.name) // Extract only `name`
      .filter((name) => name.toLowerCase().startsWith(searchText.toLowerCase()));
...
const handleSubmit = async (e) => {
 ...
 <button
  ...
  <input
    ...
    ref={locationRef}
    onChange={handleChangeStock}
    onKeyDown={handleKeyDownLocation} // Add this for Arrow key support
