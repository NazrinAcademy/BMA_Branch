...
import { CustomerAdd, Customerget } from "../../../apiService/customerAPI";
  ...
  const [successMsg,setSuccessMsg]=useState({create:"",update:""})
				...
				prevCustomers.map((customer) => {
					if (customer.id === editingCustomer) {
						return {
							...customer,
							...editUpdateCustomer,
							address: editUpdateCustomer.address || customer.address,
							Area: editUpdateCustomer.area || customer.area,
							State: editUpdateCustomer.state || customer.state,
							pinCode: editUpdateCustomer.pinCode || customer.pinCode,
						};
					}
					return customer;
				})
	...
	const handlePDF = () => {
		...
		Customers.forEach((Custom, index) => {
			doc.text(
				`${index + 1}-${Custom.supplierName} - PhoneNo: ${
					Custom.phoneNo
				} -Email: ${Supp.email}-Address ${Custom.address}-Area ${
					Custom.area
				}-PinCode${Supp.pinCode}-State${Custom.state}-OpeningBalance${
