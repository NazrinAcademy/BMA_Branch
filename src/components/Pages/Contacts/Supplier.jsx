...
//  -------------------Edit, Delete,View function------------------
    ...
    if (e.key === "Enter") {
      setFilteredSuppliers((prevSuppliers) =>
        prevSuppliers.map((supplier) => {
          if (supplier.id === editingSupplier) {
            return {
              ...supplier,
              ...editUpdateSupplier,
              address: editUpdateSupplier.address || supplier.address,
              area: editUpdateSupplier.area || supplier.area,
              state: editUpdateSupplier.state || supplier.state,
              pinCode: editUpdateSupplier.pinCode || supplier.pinCode,
            };
          }
          return supplier;
        })
  ...
  const confirmDeleteData =()=>{
    setFilteredSuppliers((prevData)=>prevData.filter((item)=>item.id !== selectedRowId)
  );
...
// ------------------------Export,Pdf,Print--------------------------------
  ...
  const handlePDF = () => {
    ...
    Suppliers.forEach((Supp, index) => {
      doc.text(
        `${index + 1}-${Supp.supplierName} - PhoneNo: ${Supp.phoneNo} -Email: ${
          Supp.email
        }-Address ${Supp.address}-Area ${Supp.area}-PinCode${
          Supp.pinCode
        }-State${Supp.state}-OpeningBalance${
          Supp.openingBalance
