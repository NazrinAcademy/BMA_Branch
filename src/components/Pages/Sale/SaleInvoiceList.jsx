...
const SaleInvoiceList = () => {
  ...
  const filteredInvoices = invoices.filter((invoice) => {
    return (
      (storeFilter === "" || invoice.billing_to.toLowerCase().includes(storeFilter.toLowerCase())) &&
      (mobileFilter === "" || invoice.mobile_no.toLowerCase().includes(mobileFilter.toLowerCase())) &&
      (statusFilter === "" || invoice.status.toLowerCase() === statusFilter.toLowerCase()) &&
      (searchTerm === "" || Object.values(invoice).some((value) =>
  ...
  const handleContextMenu = (event, invoice) => {
  ...
  const handleExportPDF = () => {
    ...
    const columns = ["Invoice No", "Date", "Billing To", "Mobile No", "Amount", "Payment Method", "Status"];
  ...
  const handlePrint = () => {
    ...
    printWindow.document.write("<tr><th>Invoice No</th><th>Date</th><th>Billing To</th><th>Mobile</th><th>Amount</th><th>Method</th><th>Payment Status</th></tr>");
  ...
  return (
    <div className="bg-white h-full px-7 py-3 rounded shadow-md w-full max-w-6xl font-['Plus Jakarta Sans'] mx-auto">
      <div className="flex justify-between items-center border-b py-4">
        <h2 className="text-xl font-semibold">Sale Invoice List</h2>
            ...
            <tbody>
              {paginatedInvoices.length > 0 ? (
                paginatedInvoices.map((invoice) => (
                  <tr
                    key={invoice.id}
                    className="border-t text-center text-sm cursor-pointer"
                ...
                <tr>
                  <td colSpan="8" className="py-4 text-gray-500">No data found</td>
                </tr>
              )}
            </tbody>
...
<div className="p-2">
 ...
 onClick={handlePrintInvoice }   >      
      ...
      {/* Print Invoice Modal */}
      {selectedInvoice && (
        <SaleInvoice invoice={selectedInvoice} onClose={() => setSelectedInvoice(null)} />
      )}
  </div>
