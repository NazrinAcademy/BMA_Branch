...
const SubCategoryTable = ({
  ...
  handleDelete,
  setIsEdited,
  handleEdit, // Using handleEdit from props
  setIsShowModal, // Ensure this is passed as a prop
  setForm,
  setSelectedState,
  setEditData,
  setEditDropdown,
  ...
  const categoryInputRef = useRef(null); // Define categoryInputRef
  ...
  const handleRightClick = (event, subCategory) => {
    ...
    setContextMenu({
      x: event.clientX,
      y: event.clientY,
      subCategories: subCategory, // Store the whole object
    });
  };
              ...
              {paginatedCategories.length > 0 ? (
                paginatedCategories.map((item, index) => (
                  ...
                  >
                    ...
                    <td className="p-2 text-sm w-[20%]">{item.categories}</td>
                    <td className="p-2 text-sm w-[20%]">{item.subCategories}</td>
                    <td className="p-2 text-sm w-[20%]">{item.hsnSacCode}</td>
                  </tr>
                ))
              ) : (
                <tr>
