...
import { addSubCategory, deleteSubCategory, fetchSubCategories, updateSubCategory } from "../../api/subcategoryAPI";
...
const SubCategories = () => {
  const allsubCategories = [
    { id: 1, category_name: "Fashion", subcategory_name: "Women's Collection", hsn_sac_code: 98765 },
    { id: 2, category_name: "Electronics", subcategory_name: "Men's Collection", hsn_sac_code: 45674 },
    { id: 3, category_name: "Food", subcategory_name: "Kids Collection", hsn_sac_code: 65468 },
    { id: 4, category_name: "Vegetables", subcategory_name: "Spare Parts", hsn_sac_code: 76543 },
    { id: 5, category_name: "Accessories", subcategory_name: "Machines", hsn_sac_code: 90874 },
    { id: 6, category_name: "Home Appliances", subcategory_name: "Foot Wears", hsn_sac_code: 12345, },
    { id: 7, category_name: "Cloths", subcategory_name: "Leafs", hsn_sac_code: 98464 },
    { id: 8, category_name: "Toys", subcategory_name: "Remote controls", hsn_sac_code: 78363 },
    { id: 9, category_name: "Books", subcategory_name: "Story Books", hsn_sac_code: 56838 },
    { id: 10, category_name: "Jewelry", subcategory_name: "Gold", hsn_sac_code: 47364 },
  ];

  const [subCategories, setSubCategories] = useState([]); // ✅ Default to an empty array
...
// ------------------ fetch sub category:
  ...
  useEffect(() => {
    const loadSubCategories = async () => {
      const config = {
        headers: { Authorization: `Bearer ${userDetails?.access_token}` },
      };
 ...
 const handleSaveSubCategory = async () => {
  if (!newSubCategory.subcategory_name || !newSubCategory.category_id) {
  ...
  try {
    await addSubCategory(newSubCategory, config);
    alert("Subcategory added successfully!");
    await fetchAllSubCategories(); // Refresh the list
    setIsShowModal({ add: false, edit: false });
    setNewSubCategory({ category_id: "", subcategory_name: "", hsn_sac_code: "" });
  } catch (error) {
  ...
  const handleExportPDF = () => {
    ...
    subCategories.forEach((category, index) => {
      doc.text(
        `${index + 1}. ${category.category_name} - ${category.subcategory_name} - HSN/SAC: ${category.hsn_sac_code}`,
        20,
        y
      );
      y += 10;
  ...
  const filteredCategories = Array.isArray(subCategories)
  ...
  const handleEdit = async (subcategory) => {
    console.log("Editing Subcategory:", subcategory); // Debugging
    setEditRowId(subcategory.subcategory_id);
    setNewSubCategory(subcategory); 
    setIsShowModal({ add: false, edit: true }); // Ensure modal opens
  };
...
const handleUpdateSubCategory = async () => {
  ...
  try {
    await updateSubCategory(editRowId, newSubCategory, config);
...
const handleDelete = (subCategory) => {
  setSelectedRowId(subCategory.subcategory_id); // Use correct ID
...
const confirmDelete = async () => {
  ...
  try {
    await deleteSubCategory(selectedRowId, config);
