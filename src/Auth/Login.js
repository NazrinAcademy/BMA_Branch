...
import { getSampleProduct, Loginfun, loginSales } from '../Utils/apiService';
...
const Login = () => {
  ...
  console.log('select edRole',selectedRole);
  ...
  const [isModalVisiblesales, setIsModalVisiblesales] = useState(false);
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      switch (selectedRole) {
        case "admin":
          loginData = await Loginfun(email, password); // Admin login function
          break;
  
        case "sales":
          loginData = await loginSales(email, password); // Sales login function
          break;
  
        // case "delivery":
        //   loginData = await LoginDeliveryFunction(email, password); // Delivery login function
        //   break;
  ...
  const closeModalsales = () => {
    setIsModalVisiblesales(false);
    navigation.navigate('SalesNavbaritem');
  };
  ...
  return (
    ...
    >
      ...
      <View
        style={[styles.inputContainer, activeField === 'email' && styles.activeInputContainer]}
      >
        <User size={24} color="rgba(131, 131, 131, 1)" style={styles.icon} />
        <TextInput
          style={styles.input}
          placeholder={activeField === ' User Name' ? '' : 'email'}
      ...
      <TouchableOpacity style={styles.loginButton} onPress={logout2}>
        <Text style={styles.loginButtonText}>Login2</Text>
      </TouchableOpacity>
      ...
      {/* sales */}
      <Modal
        visible={isModalVisiblesales}
        transparent={true}
        animationType="fade"
        onRequestClose={closeModalsales}
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <Image
            ...
            <Text style={styles.modalTitle}>Success!</Text>
            <Text style={styles.modalMessage}>Login successful! Welcome back to Wood Jungle!</Text>
            <TouchableOpacity style={styles.modalButton} onPress={closeModalsales}>
              <Text style={styles.modalButtonText}>Continue</Text>
            </TouchableOpacity>
          </View>
        </View>
