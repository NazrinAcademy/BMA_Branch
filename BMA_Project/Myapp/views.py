...
async def async_add_product(request):
    try:
        ...
        required_fields = [
            ...
            'unit', 'bar_oq_code', 'description', 'product_code'
        ]
        for field in required_fields:
            if not data.get(field):
                return JsonResponse({'error': f'Missing required field: {field}'}, status=400)
        ...
        # if not image_file:
        #     return JsonResponse({'error':f'img file is missing'})
        # try:
        #     upload_img = await sync_to_async(cloudinary.uploader.upload)(image_file)
        #     image_url = upload_img.get('secure_url')           
        # except  Exception as e:
        #     return JsonResponse({'error':f'failed to upload images : str{e}'},status = 500)
...
#         if not product_name:
#             return JsonResponse({'error': 'The product_name query parameter is required'}, status=400)
#         if not ObjectId.is_valid(product_name):
#             return JsonResponse({'error': 'Invalid ObjectId format'}, status=400)
#         try:
#             product = await sync_to_async(ProductDetails.objects.get)(product_name=product_name)
#         except ProductDetails.DoesNotExist:
#             return JsonResponse({'error': 'Product not found'}, status=404)
#         try:
#             price_details = await sync_to_async(PriceDetails.objects.get)(product_Id= product.product_Id)
#         except PriceDetails.DoesNotExist:
#             return JsonResponse({'error':f'price Not found '},status=404)
#         product_data = {
#             'product_Id': str(product.product_Id),
...
async def async_get_product_by_id(request):
    try:
        ...
        try:
            ...
            return JsonResponse({'error':f'price Not found '},status=404)
        try:
            price_gst_details = await sync_to_async(PriceGstDetails.objects.get)(product_Id= product.product_Id)
        except PriceGstDetails.DoesNotExist:
            return JsonResponse({'error':f'price Not found '},status=404)
        try:
            Stock_details = await sync_to_async(StockDetails.objects.get)(product_Id= product.product_Id)
        except PriceGstDetails.DoesNotExist:
            return JsonResponse({'error':f'price Not found '},status=404)
        product_data = {
            'product_Id': str(product.product_Id),
            ...
            'cgst': price_gst_details.cgst,
            # 'cgstprice': price_details.cgstprice,
            'sgst': price_details.sgst,
            # 'sgstprice': price_details.sgstprice,
            'low_stock_qty': Stock_details.low_stock_qty,
        }
        return JsonResponse(product_data, status=200)
    except Exception as e:
...
grand_total =0
@csrf_exempt
def customer_product_calculation(request):
    return async_to_sync(async_Customer_product_cul)(request)
async def async_Customer_product_cul(request):
    global total_amount_before_tax, total_cgst, total_sgst, grand_total
    try:
        if request.method != 'POST':
            return JsonResponse({'error':f'Invalid request Method, only POST allowed'},status=400)
        object_id = request.GET.get('object_id')
        
        try:
            data = json.loads(request.body)
        except json.JSONDecodeError:
            return JsonResponse({'error':f'Invalid JSON payload received'},status=400)
        qyt = data.get('Qyt',0)
        try:
        ...
        sgst = qyt * gstamount.sgstprice
        discount_value = (multible_amount * discount) / 100  
        total_amount = multible_amount - discount_value + cgst + sgst
        
        grand_total += total_amount
        total_amount_before_tax += multible_amount
        total_cgst += cgst
        total_sgst += sgst
        
        # grand_total = total_amount_before_tax - discount_value + total_cgst + total_sgst
        Final_total_amount = grand_total
        return JsonResponse({
            'rate': multible_amount,
            'cgst': cgst,
            'sgst': sgst,
...
@csrf_exempt
def balance_calculation(request):
    return async_to_sync(async_balance_calculation)(request)
async def async_balance_calculation(request):
    try:
        if request.method!= 'POST':
            return JsonResponse({'error':f'Invalid requesst method , only post allowed'},status=400)
        try:
            data = json.loads(request.body)
        except json.JSONDecodeError:
        ...
        recievd = data['Received']
        Balance_amount = Total_Amount - recievd
        return JsonResponse({'balance_amout ': Balance_amount})
...
async def async_get_customer(request):
    try:
        ...
        if not customer_name:
            return JsonResponse({'error':f'customer_name requerment'},status=400)
        try:
...
def add_access_key(request):
    ...
    required_fields = ['access_key', 'name', 'email']
    for field in required_fields:
        if field not in data:
            return JsonResponse({'error': f'Missing required field: {field}'}, status=400)
    ...
    try:
        ...
        response_data = {
            ...
            'email': access.email_id,
            'name': access.name,
            'access_key': access.access_key,
            'hardware_details': hardware_details
        }

        api_url = "https://dfc9-59-92-125-2.ngrok-free.app/hardware/api/add"

        headers = {'Content-Type': 'application/json'}
        print("📤 Sending to API:", api_url)
        print("📦 Payload:", json.dumps(response_data, indent=2))

        try:
            api_response = requests.post(api_url, json=response_data, headers=headers, timeout=10)
            print("🔄 API Response:", api_response.status_code, api_response.text)  

            api_response.raise_for_status()  # Raise exception if response code is not 2xx
            external_api_msg = api_response.json()  # Assuming the API returns a JSON response
        except requests.RequestException as e:
            external_api_msg = {'error': f'Failed to send data to external API: {str(e)}'}

        # Return response to frontend including external API response
        return JsonResponse({
            'local_response': response_data,
            'external_api_response': external_api_msg
        }, status=200)

    except IntegrityError:
        return JsonResponse({'error': 'AccessKey ID already exists'}, status=400)
    except Exception as e:
        return JsonResponse({'error': f'An unexpected error occurred: {str(e)}'}, status=500)
...
def get_hardware_details():
    try:
        ...
        c = wmi.WMI()
        motherboard_id = c.Win32_BaseBoard()[0].SerialNumber
        ...
        return {"error": f"Hardware info retrieval failed: {str(e)}"}

@csrf_exempt
def verify_code(request):
    return async_to_sync(update_accesskey)(request)
async def update_accesskey(request):
    if request.method != 'PUT':
        return JsonResponse({'error': 'Invalid request method, only PUT allowed'}, status=400)
    ...
    if not data:
        return JsonResponse({'error': 'Missing all fields'}, status=400)
    ...
    try:
        check_email = await sync_to_async(accesskey.objects.get)(email_id=data['email'])

        check_email.permissin = data['message']
        await sync_to_async(check_email.save)()
...
def accesskey_get(request):
    return async_to_sync(async_get_accesskey)(request)
async def async_get_accesskey (request):
    try:
        if request.method != 'GET':
            return JsonResponse({'error':f'Invalid request method'},status=400)
        
        access_id = request.GET.get('object_id')
        
        if not access_id:
            return JsonResponse({'error':f'requesrment access key'},status=400)   
        try:
            datas = await sync_to_async(accesskey.objects.get)(access_id = access_id)
            print(f'data not found result {datas}')
        except accesskey.DoesNotExist:
            return JsonResponse({'error':f'Data not found'},status=404)
        access_data = {
            'id':datas.access_id,
...
async def async_update_accesskey(request):
    try:
        if request.method != 'PUT':
            return JsonResponse({'error':f'Invalid requement Method. only PUT allowed'},status=400)
        access = request.GET.get('object_id')
        
        if not access:
            return JsonResponse({'error':f' Missing Object id requrment'},status=400)
        try:
            get_datas =  await sync_to_async(accesskey.objects.get)(access_id = access)
        except accesskey.DoesNotExist:
            return JsonResponse({'error':f'data not found'},status=404)
        try:
            data = json.loads(request.body)
        except json.JSONDecodeError:
            return JsonResponse({'error':f'Invalid Json payload'},status=400)
        request_field = ['name','email','access_key']
        for field in request_field:
            if field not in data:
                return JsonResponse({'error':f'Missing all requirment fields'},status=400)
        update_fields = {}
        print(f'update data{update_fields}')
        for field in request_field:
            if field in data:
                setattr(get_datas, field, data[field])
                update_fields[field] = data[field]
        await sync_to_async(get_datas.save)()
...
async def async_delete_accesskey(request):
    try:
        if request.method != 'DELETE':
            return JsonResponse({'error':f'Invalid request Method, only Delete allowed'},status=400)
        access = request.GET.get('object_id')
        ...
        if not access:
            return JsonResponse({'error':f'Missinf access key id'},status=400)
        try:
            delete_data = await sync_to_async(accesskey.objects.get)(access_id=access)
        except accesskey.DoesNotExist:
            return JsonResponse({'error':f'Data not found'},status=404)
